...
# # # استيراد وظائف تحسين الاستعلامات
# from query_refinement import set_query_refinement_global_variables, get_ranked_query_suggestions, initialize_queries_db, index_query
...
# # دالة لتحديث الاقتراحات بناءً على النص المدخل
...
# # دالة لعرض الاقتراحات في قائمة منسدلة
...
#     suggestion_listbox.place(relx=0.5, rely=0.5, anchor=tk.CENTER)

# # دالة لتنفيذ عملية البحث
# def search():
...
#     try:
#         results = get_search_result(selected_query, dataset, retrieving_relevant_on)
#         show_results_window(selected_query, results)
#     except Exception as e:
#         show_error_window(f"An error occurred: {e}")

# # دالة لعرض نافذة النتائج
...
# label = ttk.Label(root, text="Enter your query", background="#e0f7fa")
# label.place(relx=0.5, rely=0.4, anchor=tk.CENTER)

# entry = ttk.Entry(root, width=150)
# entry.place(relx=0.5, rely=0.45, anchor=tk.CENTER)
# entry.bind("<KeyRelease>", update_suggestions)

# suggestion_listbox = tk.Listbox(root, width=150, height=10)
# suggestion_listbox.place(relx=0.5, rely=0.55, anchor=tk.CENTER)
...
#           bordercolor=[("active", "darkblue")])

# result_text = tk.StringVar()
# result_label = ttk.Label(root, textvariable=result_text, background="#e0f7fa")
# result_label.place(relx=0.5, rely=0.65, anchor=tk.CENTER)

# root.mainloop()
# # ui.pyimport tkinter as tk


# import tkinter as tk
# from tkinter import ttk
# from query_refinement import get_ranked_query_suggestions
# from search import get_search_result
